    ...
    for j in train_by_class:
        ...
        if train_by_class[j][0]*ActivationPN <= 0:
            for key, value in WeightPN.items():
                ...
                if MaxIter < MaxIterAvg:
                    WeightPN1[key] += train_by_class[j][0]*train_by_class[j][2][key]
        ...
        if train_by_class[j][1]*ActivationTD <= 0:
            for key, value in WeightTD.items():
                ...
                if MaxIter < MaxIterAvg:
                    WeightTD1[key] += train_by_class[j][1]*train_by_class[j][2][key]
...
for key, value in WeightAvgPN.items():
    WeightAvgPN[key] = WeightPN1[key] - (UpdatePN[key]/Token)

for key, value in WeightAvgTD.items():
    WeightAvgTD[key] = WeightTD1[key] - (UpdateTD[key]/Token)
